...
def get_pack_context() -> dict:
    ...
    cmd = [
        "python3",
        "src/writing_context_rtfm/cli.py",
        "pack",
        "--project-root",
        PROJECT_ROOT,
        "--corpus",
...
def build_prompt(
    context: str, instruction: str, constraints: list = None, avoid: list = None
) -> str:
    """Builds the prompt for the writing agent."""
    prompt = "You are an academic writing assistant. Your task is to write a manuscript section based on the provided context.\n\n"
    ...
    prompt += context + "\n\n"
    prompt += (
        "Write ONLY the manuscript section text. Do not include introductory conversational filler."
    )
    return prompt
