...
class OpenAISemanticSearchProvider(BaseContextProvider):
    ...
    def _get_api_key(self, store) -> Optional[str]:
    ...
    def sync_chunks(self, store, rtfm_db_path: str) -> None:
        ...
        try:
            ...
            logger.error("numpy is required for OpenAISemanticSearchProvider. Install with `pip install numpy`.")
            return

        api_key = self._get_api_key(store)
        if not api_key:
            logger.error("OpenAI API key not found. Please set OPENAI_API_KEY or use 'writing-context-rtfm auth openai_semantic <key>'")
            return
            ...
            for idx in top_k_indices:
                ...
                if row:
                    spans.append(SourceSpan(
                        ...
                        reason=f"OpenAI semantic match for query",
                        score=score,
                        priority="supporting",
