...
from writing_context_rtfm.latex import build_reference_graph, scan_latex_commands
...
class TestLatexParser(unittest.TestCase):
    ...
    def test_initialize_section_cards_dependency_resolution(self):
        ...
        intro_tex = self.project_dir / "intro.tex"
        intro_tex.write_text(
            r"\section{Intro}\label{sec:intro} \ref{sec:results}", encoding="utf-8"
        )

        results_tex = self.project_dir / "results.tex"
        results_tex.write_text(
            r"\section{Results}\label{sec:results} \ref{sec:intro}", encoding="utf-8"
        )
        ...
        with patch("writing_context_rtfm.server.load_config", return_value=mock_config):
            req = {
                ...
                "params": {
                    ...
                    "arguments": {"project_root": str(self.project_dir)},
                },
                "id": "1",
            }
