...
def test_zotero_provider_is_available():
    ...
    config = AppConfig(
        ...
        providers={
            "zotero": ProviderConfig(enabled=True, mcp_server=MCPServerConfig(command="zotero-mcp"))
        },
    )
...
def test_zotero_provider_fetch_context():
    ...
    with patch(
        "writing_context_rtfm.providers.local.get_shared_manager", return_value=mock_manager
    ):
