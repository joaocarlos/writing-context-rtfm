...
class DummyAdapter:
    ...
    def context(
        self, path: str, line_start: int, line_end: int, corpus: str = "default"
    ) -> list[Any]:
        return []
...
def test_should_auto_escalate_on_mathematical_cues():
    ...
    dummy_span = SourceSpan(
        ...
        metadata={"snippet": "Proof of Theorem 1"},
    )
...
def test_should_auto_escalate_on_lexical_uncertainty():
    ...
    low_score_spans = [
        ...
        SourceSpan(
            ...
            priority="supporting",
            source_role="reference",
            metadata={"snippet": "Background"},
        ),
    ]
    should_esc, reason = generator._should_auto_escalate(
        "Ajustar a introdução geral", None, low_score_spans
    )
    ...
    should_esc, reason = generator._should_auto_escalate(
        "Atualizar agradecimentos institucionais", None, distinct_spans
    )
...
def test_generator_auto_profile_execution(tmp_path):
    ...
    generator = ContextPackGenerator(
        ...
        section_cards=SectionCards(
            version=1, document=DocumentCard(title="Doc", thesis="Doc thesis"), sections={}
        ),
    ...
    pack = generator.generate(
        task="Agradecimentos institucionais aos colaboradores", target=None, token_budget=2000
    )
    ...
    pack_math = generator.generate(
        task="Verificar a prova do teorema 4 e limitante assintótico",
        target=None,
        token_budget=2000,
    )
