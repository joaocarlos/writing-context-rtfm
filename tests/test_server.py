...
def test_handle_get_writing_context_pack(
...
def test_handle_get_proofreading_context_pack(mock_generator_class, mock_load_runtime):
    ...
    res = handle_get_proofreading_context_pack(
        {"target_file": "file.tex", "line_start": 1, "line_end": 10}
    )
...
def test_handle_refresh_index(
    mock_resolve, mock_store_class, mock_fingerprint, mock_adapter_class, mock_load_config
):
    ...
    msg = json.dumps(
        {
            ...
            "id": 1,
            "method": "tools/call",
            "params": {"name": "get_writing_context_pack", "arguments": {"task": "test"}},
        }
    )
    process_message(msg)
